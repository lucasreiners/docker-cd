...
// DetectDrift compares desired state against runtime labels and returns drift results.
func (r *Reconciler) DetectDrift(desired []desiredstate.StackRecord, runtime map[string]StackSyncMetadata) []DriftResult {
	...
	for _, d := range desired {
		...
		if r.deployDir != "" && !isInDeployScope(d.Path, r.deployDir) {
			continue
		}

		rt, exists := runtime[d.Path]
		if !exists {
		...
		if rt.DesiredComposeHash != d.ComposeHash {
			results = append(results, DriftResult{
				Path:     d.Path,
				NeedSync: true,
				Reason:   fmt.Sprintf("compose hash drift: runtime=%s desired=%s", rt.DesiredComposeHash, d.ComposeHash),
			})
			continue
		}
	...
	if r.policy.RemoveEnabled {
		for path := range runtime {
			if !desiredPaths[path] {
				if r.deployDir != "" && !isInDeployScope(path, r.deployDir) {
					continue
				}
				results = append(results, DriftResult{
					Path:       path,
...
func (r *Reconciler) Reconcile(ctx context.Context) []ReconciliationRun {
	...
	drifts := r.DetectDrift(snap.Stacks, runtime)
	...
	for _, drift := range drifts {
		...
		for _, st := range snap.Stacks {
			if st.Path == drift.Path && st.Status != desiredstate.StackSyncSynced {
				...
				r.updateStackSynced(drift.Path, rt.DesiredRevision, rt.DesiredCommitMessage, rt.DesiredComposeHash, rt.SyncedAt)
				break
			}
		}
	...
	for _, drift := range drifts {
		...
		if r.policy.DriftPolicy == "flag" {
			if !r.ackStore.IsAcknowledged(drift.Path) {
				...
				r.updateStackStatus(drift.Path, desiredstate.StackSyncFailed, "", "drift detected, awaiting acknowledgement")
				continue
			}
...
func (r *Reconciler) syncStack(ctx context.Context, drift DriftResult, snap *desiredstate.Snapshot) ReconciliationRun {
	...
	r.updateStackStatus(drift.Path, desiredstate.StackSyncSyncing, "", "")
	...
	if err != nil {
		...
		run.FinishedAt = time.Now()
		r.updateStackStatus(drift.Path, desiredstate.StackSyncFailed, "", run.Error)
		return run
	}
	defer cleanup()
	...
	if err != nil {
		...
		r.updateStackStatus(drift.Path, desiredstate.StackSyncFailed, "", truncateError(run.Error))
		return run
	}
...
func (r *Reconciler) removeStack(ctx context.Context, drift DriftResult, snap *desiredstate.Snapshot) ReconciliationRun {
	...
	r.updateStackStatus(drift.Path, desiredstate.StackSyncDeleting, "", "")
	...
	r.updateStackStatus(drift.Path, desiredstate.StackSyncMissing, "", "")

	return run
}

func (r *Reconciler) updateStackStatus(path string, status desiredstate.StackSyncStatus, syncedAt, syncError string) {
	snap := r.store.Get()
	if snap == nil {
		return
	}
...
func (r *Reconciler) updateStackSynced(path, revision, commitMessage, composeHash, syncedAt string) {
	snap := r.store.Get()
	if snap == nil {
		log.Printf("[warn] updateStackSynced: store snapshot is nil for stack %s", path)
		return
	}
	...
	if !found {
		log.Printf("[warn] updateStackSynced: stack %s not found in snapshot (stacks: %d)", path, len(snap.Stacks))
	} else {
