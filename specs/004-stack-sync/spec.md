...
- Q: Which runtime resources should carry sync metadata labels? → A: Containers only.
- Q: Why keep per-stack compose hashes when we already have Git revision? → A: Hashes provide stack-level granularity so unchanged stacks are not reconciled on every revision.
- Q: How should operator acknowledgement be provided when drift policy is "flag"? → A: Via an HTTP acknowledgement endpoint that accepts a stack path.
...
- **FR-006**: System MUST record, per stack, the last synced desired revision, commit message, and compose hash in a durable way across restarts.
...
- **FR-021**: System MUST store sync metadata as labels on stack containers so it survives service restarts.
- **FR-022**: System MUST define a drift policy (revert or flag) and apply it consistently during reconciliation.
- **FR-023**: When drift policy is set to "flag", system MUST surface drift and require explicit operator acknowledgement before reconciliation proceeds for that stack.
- **FR-024**: System MUST expose an acknowledgement endpoint that accepts a stack path and records operator approval for reconciliation.
